...
function isInDocument(node) {
  return node.ownerDocument && containsNode_1$1(node.ownerDocument.documentElement, node);
}

function getActiveElementDeep() {
  var win = window;
  var element = getActiveElement_1$1();
  while (element instanceof win.HTMLIFrameElement) {
    ...
    element = getActiveElement_1$1(win.document);
  }
  return element;
}
...
function getElementsWithSelections(acc, win) {
  ...
  var element = getActiveElement_1$1(doc);
...
function restoreSelection(priorSelectionInformation) {
  ...
  elementSelections.forEach(function (selection) {
    ...
    if (isInDocument(element) && getActiveElement_1$1(element.ownerDocument) !== element) {
...
function constructSelectEvent(nativeEvent, nativeEventTarget) {
  ...
  if (mouseDown || activeElement$1 == null || activeElement$1 !== getActiveElement_1$1(doc)) {
    return null;
  }
