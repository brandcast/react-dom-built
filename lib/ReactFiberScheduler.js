...
var TaskPriority = _require2.TaskPriority;
var AnimationPriority = _require2.AnimationPriority;
var HighPriority = _require2.HighPriority;
...
var Callback = _require3.Callback;
var PlacementAndCallback = _require3.PlacementAndCallback;
var UpdateAndCallback = _require3.UpdateAndCallback;
var PlacementAndUpdateAndCallback = _require3.PlacementAndUpdateAndCallback;
var DeletionAndCallback = _require3.DeletionAndCallback;

var _require4 = require('./ReactTypeOfWork');

var HostContainer = _require4.HostContainer;
var ClassComponent = _require4.ClassComponent;
...
module.exports = function (config) {
  ...
  var _ReactFiberCommitWork = ReactFiberCommitWork(config, trapError);
  ...
  var hostScheduleDeferredCallback = config.scheduleDeferredCallback;
  var useSyncScheduling = config.useSyncScheduling;
  ...
  var shouldBatchUpdates = false;

  // Need this to prevent recursion while in a Task loop.
  var isPerformingTaskWork = false;
  ...
  var activeErrorBoundaries = null;
  var nextTrappedErrors = null;
  ...
  function performAnimationWorkUnsafe() {
    ...
    nextUnitOfWork = findNextUnitOfWork();
    while (nextUnitOfWork && nextPriorityLevel === AnimationPriority) {
      nextUnitOfWork = performUnitOfWork(nextUnitOfWork);
      if (!nextUnitOfWork) {
  ...
  function performSynchronousWorkUnsafe() {
    nextUnitOfWork = findNextUnitOfWork();
    while (nextUnitOfWork && nextPriorityLevel === SynchronousPriority) {
      nextUnitOfWork = performUnitOfWork(nextUnitOfWork);
  ...
  function performTaskWorkUnsafe() {
    ...
    try {
      nextUnitOfWork = findNextUnitOfWork();
      while (nextUnitOfWork && nextPriorityLevel === TaskPriority) {
        nextUnitOfWork = performUnitOfWork(nextUnitOfWork);
  ...
  function performAndHandleErrors(priorityLevel, deadline) {
    ...
    while (true) {
      ...
      if (nextTrappedErrors && !activeErrorBoundaries) {
        var nextUncaughtError = handleErrors();
  ...
  function handleErrors() {
    ...
    while (nextTrappedErrors && nextTrappedErrors.length && !firstUncaughtError) {
      ...
      while (nextTrappedErrors && nextTrappedErrors.length) {
        var trappedError = nextTrappedErrors.shift();
        var _boundary = trappedError.boundary;
        var _error = trappedError.error;
        var _root2 = trappedError.root;
        if (!_boundary) {
          firstUncaughtError = firstUncaughtError || _error;
          if (_root2 && _root2.current) {
            ...
            _root2.current.pendingWorkPriority = NoWork;
          } else {
        ...
        if (activeErrorBoundaries.has(_boundary)) {
          continue;
        }
        try {
  ...
  function trapError(failedFiber, error, isUnmounting) {
    ...
    while (fiber) {
      ...
      if (parent) {
        if (parent.tag === ClassComponent && boundary === null) {
          ...
          var isBoundary = typeof instance.unstable_handleError === 'function';
          if (isBoundary) {
            // Skip boundaries that are already active so errors can propagate.
            var isBoundaryAlreadyHandlingAnotherError = activeErrorBoundaries !== null && activeErrorBoundaries.has(parent);
  ...
  function scheduleCallbackAtPriority(priorityLevel) {
    switch (priorityLevel) {
      case SynchronousPriority:
    ...
    while (true) {
      ...
      if (!fiber['return']) {
        if (fiber.tag === HostContainer) {
          var _root3 = fiber.stateNode;
          scheduleWorkAtPriority(_root3, priorityLevel);
          return;
        } else {
          throw new Error('Invalid root');
        }
