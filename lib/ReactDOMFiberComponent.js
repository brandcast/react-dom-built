...
var isEventSupported = require('fbjs/lib/isEventSupported');
...
function updateDOMProperties(domElement, rootContainerElement, lastProps, nextProps, wasCustomComponentTag, isCustomComponentTag) {
  ...
  if (styleUpdates) {
    ...
    CSSPropertyOperations.setValueForStyles(domElement, styleUpdates, componentPlaceholder // TODO: Change CSSPropertyOperations to use getCurrentOwnerName.
    );
  }
}
...
var ReactDOMFiberComponent = {
  ...
  isNewHostContainer: function (tag) {
    return tag === 'svg' || tag === 'foreignobject';
  },
  createElement: function (tag, props, rootContainerElement) {
    ...
    if (namespaceURI == null || namespaceURI === DOMNamespaces.svg && rootContainerElement.tagName === 'foreignObject') {
    ...
    if (namespaceURI === DOMNamespaces.html) {
      ...
      domElement = ownerDocument.createElementNS(namespaceURI, tag);
    }
  ...
  setInitialProperties: function (domElement, tag, rawProps, rootContainerElement) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (isCustomComponentTag && !didWarnShadyDOM && domElement.shadyRoot) {
        process.env.NODE_ENV !== 'production' ? warning(false, '%s is using shady DOM. Using shady DOM with React can ' + 'cause things to break subtly.', getCurrentOwnerName() || 'A component') : void 0;
